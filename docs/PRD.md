...
* **[FR-1.4] 날짜 셀 선택 하이라이트 일치**:
  * 일간, 주간, 월간 모든 뷰 모드에서 캘린더의 선택 날짜(또는 오늘 날짜)를 강조하는 하이라이트는 일관된 형태로 표현되어야 함.
  * 뷰 모드와 무관하게 하이라이트의 모양은 항상 **원형(CircleShape)**, 지름 크기는 항상 **`26.dp`**로 균일하게 통일하여 시각적 일관성을 확보함.
